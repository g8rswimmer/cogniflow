...
| DT-02 | **Conditional** | Evaluate a boolean CEL (Common Expression Language) expression against the current workflow data context using `google/cel-go`. Route execution to one of two downstream branches (true / false). Expressions are validated at save time before execution. |
