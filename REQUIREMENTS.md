...
| WF-09 | A workflow may declare an optional **initial data schema** — a JSON Schema object defining the expected field names, types, and descriptions for the run's `_initial` data. The schema is advisory: it assists workflow authors and drives the template variable picker (surfacing declared fields as individual `{{._initial.field}}` chips) but does not block execution when fields are absent or mismatched. When a schema is defined, the Run modal renders a guided form (RJSF) instead of a free-form JSON textarea. |
...
| UI-13 | When no canvas node is selected, the Config Sidebar displays a **Workflow Settings** panel containing a **Workflow Inputs** section. Users can define the expected initial data fields (name, type, optional description) via a row editor that mirrors the Output Parser UI. Fields can be added, retyped, and deleted. Saving the workflow persists the schema. |
| UI-14 | When a workflow has a declared initial data schema (WF-09) and the user clicks **Run**, the Initial Data modal renders a generated RJSF form with a labeled, type-appropriate input per declared field instead of the free-form JSON textarea. When no schema is defined, the modal falls back to the existing JSON textarea (backward compatible). |
