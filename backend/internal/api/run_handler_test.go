...
func TestRunHandler_ListRuns_WorkflowNotFound(t *testing.T) {
	h, _, _ := setupRunHandler(t)

	r := httptest.NewRequest("GET", "/workflows/nonexistent/runs", nil)
...
func TestRunHandler_TriggerRun_MalformedBody(t *testing.T) {
	...
	r := httptest.NewRequest("POST", "/workflows/wf-1/runs", strings.NewReader(`{bad json`))
	r.SetPathValue("id", "wf-1")
	r.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	h.triggerRun(w, r)
